...
<body>
  ...
  <div id="output">
    <div class="snap-grid">
      <div class="snap-card"><div class="snap-label">Mission Geometry</div><div class="snap-value">Not loaded</div><div class="snap-sub">Draw a rectangle or line.</div></div>
  ...
  <script>
    ...
    function snapshotHtml(summary) {{
      ...
      if (summary.geometry_type === 'rectangle') {{
        return '<div class="snap-grid">' +
          '<div class="snap-card"><div class="snap-label">Mission Geometry</div><div class="snap-value">Search Area</div><div class="snap-sub">Rectangle captured for ISR / Area Search / MCM.</div></div>' +
...
def build_distribution_chart(energy_arr: np.ndarray, p80: float, usable_total_kwh: float) -> Any:
    ...
    spread = max(p99 - p01, 0.05)
    xmin = max(0.0, p01 - 0.35 * spread)
    xmax = p99 + 0.45 * spread

    ax.hist(energy_arr, bins=15, alpha=0.85)
    ax.axvline(p80, linestyle="--", label="P80 energy")

    # Do not stretch the chart just to show a very large battery inventory line.
    if usable_total_kwh <= xmax * 1.15:
        ax.axvline(usable_total_kwh, linestyle=":", label="Battery inventory without recharge")
        xmax = max(xmax, usable_total_kwh * 1.05)
    else:
        ax.text(
            0.98,
            0.95,
            f"Battery inventory without recharge: {usable_total_kwh:.1f} kWh",
...
def build_search_overlay_chart(summary: Dict[str, Any], context: Optional[Dict[str, Any]], track_spacing_m: float) -> Any:
    """Simple planning-view overlay of the selected rectangle and generated swath lanes.

    This is not a geodetic tactical map. It is a mission-planning view in local km
    coordinates that shows the recommended track orientation and spacing concept.
    """
    fig, ax = plt.subplots(figsize=(7, 4))
    ...
    ax.plot(rect_x, rect_y, linewidth=2, label="Selected mission area")

    if orientation == "North-South":
        xs = np.arange(spacing_km / 2, width, spacing_km)
        for idx, x in enumerate(xs):
            y0, y1 = (0, height) if idx % 2 == 0 else (height, 0)
            ax.plot([x, x], [y0, y1], linewidth=1, alpha=0.65)
            # small direction arrow
            mid = height * 0.5
            dy = height * 0.12 if y1 > y0 else -height * 0.12
            ax.arrow(x, mid - dy / 2, 0, dy, length_includes_head=True, head_width=max(width * 0.006, 0.02),
                     head_length=max(height * 0.025, 0.02), alpha=0.65)
    else:
        ys = np.arange(spacing_km / 2, height, spacing_km)
        for idx, y in enumerate(ys):
            x0, x1 = (0, width) if idx % 2 == 0 else (width, 0)
            ax.plot([x0, x1], [y, y], linewidth=1, alpha=0.65)
            mid = width * 0.5
            dx = width * 0.12 if x1 > x0 else -width * 0.12
            ax.arrow(mid - dx / 2, y, dx, 0, length_includes_head=True, head_width=max(height * 0.006, 0.02),
                     head_length=max(width * 0.025, 0.02), alpha=0.65)

    # Current arrow in lower-left corner
    ...
    if cur_speed > 0:
        ...
        length = max(min(width, height) * 0.18, 0.05)
        x0, y0 = width * 0.10, height * 0.12
        ax.arrow(x0, y0, length * math.cos(theta), length * math.sin(theta),
                 length_includes_head=True, head_width=max(min(width, height) * 0.025, 0.03),
                 head_length=max(min(width, height) * 0.035, 0.04), linestyle="--")
        ax.text(x0, y0 + max(height * 0.08, 0.05), f"Current {cur_speed:.2f} kts @ {cur_dir:.0f}°", fontsize=9)

    ax.set_title(f"Recommended Search Pattern Overlay: {orientation}")
    ...
    ax.text(
        0.01, -0.16,
        f"Planning overlay only. Swath/track spacing: {spacing_km:.3f} km. Lines show generated coverage pattern, not tactical navigation orders.",
        transform=ax.transAxes,
        fontsize=8,
        va="top",
    )
...
def run_simulation_core(
    ...
    return status, results_df, equivalents_df, fig_energy_time, fig_dist, fig_overlay, summary, results_html, metoc
...
def run_from_ui(
    ...
    status, df, equiv_df, fig_time, fig_dist, fig_overlay, summary, results_html, metoc = run_simulation_core(
    ...
    return status, df, equiv_df, fig_time, fig_dist, fig_overlay, summary, results_html, metoc
