...
def power_model_breakdown(
    ...
    p_nom = max(float(vehicle.average_power_kw), 0.001)
    ...
    bounded_hotel_fraction = _clamp(selected_hotel_fraction, 0.20, 0.80)
    p_hotel = p_nom * bounded_hotel_fraction
    p_prop_nom = max(p_nom - p_hotel, 0.0)
    p_prop = p_prop_nom * (v / v_nom) ** float(speed_exponent) * max(float(propulsion_multiplier), 0.0)
    ...
    if v < min_efficient_speed:
        penalty_fraction = max(_vehicle_float(vehicle, ("low_speed_penalty_fraction",), 0.15), 0.0)
        raw_penalty = p_nom * penalty_fraction * ((min_efficient_speed - v) / min_efficient_speed) ** 2
