...
| Public-facing Navy/program variant entries | Planning baseline | Active vehicle-catalog source notes and associated public specification sheets | Catalog values require independent source verification before operational use or publication as authoritative specifications. |
...
| Stochastic current speed | Model assumption | `core/energy.py` bounded-normal sampler | Each Monte Carlo mission sequence uses an independent current-speed draw with `sigma = max(0.02 kts, 25% of mean)` and bounds at mean plus or minus two sigma. Current direction remains fixed. |
| Sustainment horizon variance | Model assumption | `core/sustainment.py` mission/day resampling | The horizon independently resamples the normalized single-mission Monte Carlo distribution and preserves the unrounded planning-recommendation baseline. This is not a tide or weather forecast. |
